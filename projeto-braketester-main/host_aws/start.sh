...
function clearContainers() {
  CONTAINER_IDS=$(docker container ls | awk '{print $1}')
  if [ -z "$CONTAINER_IDS" -o "$CONTAINER_IDS" == " " ]; then
    infoln "No containers available for deletion"
  else
    ./scripts/teardown.sh ${COMPOSE_FILE_BASE}
  fi
}
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Orgs Identities"

    set -x
    cryptogen generate --config crypto-config-nmi.yaml --output=crypto-config
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
...
function generateMSP() {
  ...
  configtxgen -profile NMIGenesis -outputBlock ./channel-artifacts/genesis.block -channelID sys-channel
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
## Call the script to test a chaincode
function testCC() {
  scripts/testCC.sh $CHANNEL_NAME $CC_NAME $Args

  if [ $? -ne 0 ]; then
    fatalln "Testing chaincode failed"
  fi
}
...
COMPOSE_FILE_BASE=peer-inmetro_aws.yaml
...
if [ "$MODE" == "up" ]; then
...
elif [ "$MODE" == "generateMSP" ]; then
  infoln "generating MSP"
  ...
  infoln "testing chaincode on channel '${CHANNEL_NAME}'"
else
  printHelp
  exit 1
fi
