...
type SensorLSH struct {
    ...
    Total_of_seconds			float64	`json:"total_of_seconds"`
    Signal_data			string	`json:"signal_data"`
    Hash_lsh					string	`json:"hash_lsh`
    Hash_crip					string	`json:"hash_crip`
    }
...
func (s *SmartContract) Invoke(stub shim.ChaincodeStubInterface) sc.Response {
	...
	}else if fn == "insertMeasurementLSH" {
		//look for a specific fill up record and brings its changing history
		return s.insertMeasurementAWS(stub, args)
...
func (s *SmartContract) insertMeasurementLSH(stub shim.ChaincodeStubInterface, args []string) sc.Response {
